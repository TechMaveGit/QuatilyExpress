...
class DynamicSelect {
        ...
        let template = `
                <div class="dynamic-select ${this.name}"${this.selectElement.id ? ' id="' + this.selectElement.id + '"' : ''} style="${this.width ? 'width:' + this.width + ';' : ''}${this.height ? 'height:' + this.height + ';' : ''}">
                    ...
                    <div class="dynamic-select-header" style="${this.width ? 'width:' + this.width + ';' : ''}${this.height ? 'height:' + this.height + ';' : ''}">
                        <input type="text" class="dynamic-select-search form-control" placeholder="Search...">
                        <span class="dynamic-select-header-placeholder">${this.placeholder}</span>
                    </div>
                    <div class="dynamic-select-options" style="${this.options.dropdownWidth ? 'width:' + this.options.dropdownWidth + ';' : ''}${this.options.dropdownHeight ? 'height:' + this.options.dropdownHeight + ';' : ''}">
                        <!-- Search input -->
                        <input type="text" class="dynamic-select-search form-control" placeholder="Search...">
                        ${optionsHTML}
                    </div>
    ...
    _eventHandlers() {
        this.element.querySelector('.dynamic-select-header').addEventListener('click', () => {
            this.element.querySelector('.dynamic-select-search').focus();
        });
    
        this.element.querySelector('.dynamic-select-search').addEventListener('input', () => {
            let searchTerm = this.element.querySelector('.dynamic-select-search').value.toLowerCase();
            this.element.querySelectorAll('.dynamic-select-option').forEach(option => {
                let optionText = option.querySelector('.dynamic-select-option-text').innerHTML.toLowerCase();
        ...
        this.element.querySelectorAll('.dynamic-select-search').forEach(searchInput => {
